...
object _00KuduStreamApp extends BasicStreamApp {
	...
	override def process(streamDF: DataFrame, category: String): DataFrame = {
		...
		category match {
			...
			// 其他业务系统处理转换
			case _ =>
				// TODO: 基于DSL编程，使用SQL函数，解析JSON为Bean对象
				// a. 自定义UDF函数
				val json_to_bean: UserDefinedFunction = udf(
					(message: String) => {
						JSON.parseObject(message, classOf[CanalMessageBean])
					}
				)
				// b. 转换数据
				val beanDF: DataFrame = streamDF
					// 过滤数据
    				.filter($"value".isNotNull && length(trim($"value")).gt(0))
					// 应用udf函数，转换json为bean对象
    				.select(
					    json_to_bean(trim($"value")).as("bean")
				    )
					// 获取所有字段
    				.select($"bean.*")
				// c. 返回数据
