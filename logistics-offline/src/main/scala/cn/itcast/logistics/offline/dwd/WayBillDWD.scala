...
object WayBillDWD extends AbstractOfflineApp {
  /**
   * 数据拉宽，将事实表与维度表进行关联LeftJoin拉宽操作
   */
  override def process(dataframe: DataFrame): DataFrame = {
    // 此处dataframe：表示事实表数据，就是运单表数据
    import dataframe.sparkSession._
    ...
    // 第三步、选取字段，并且添加字段day日期，表示数据属于哪一天
  ...
  def main(args: Array[String]): Unit = {
    // 直接调用父类，模板方法即可，传递参数值
    execute(
      this.getClass, TableMapping.WAY_BILL, //
